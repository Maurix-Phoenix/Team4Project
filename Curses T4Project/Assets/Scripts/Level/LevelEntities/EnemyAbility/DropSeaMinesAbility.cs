...
public class DropSeaMines : MonoBehaviour
{
    ...
    {
        if (!gameObject.GetComponent<EnemyShip>().IsStopped && !gameObject.GetComponent<EnemyShip>().IsDead && (gameObject.transform.position.x < 20 && gameObject.transform.position.x > 0))
        {
            if (_IsMineCounted)
            {
                if (_MineToDrop > 0)
